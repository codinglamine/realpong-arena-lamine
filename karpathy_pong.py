""" karpathy_pong.py  --  Karpathy policy-gradient (the original algorithm) with an
    IMPROVED training stack, on pong_v3. Trains agent_ale.PolicyNet -> karpathy_pong.pt
    which the arena loads directly:  python arena_ale.py karpathy_pong.pt bf
    ...
    Run:  python karpathy_pong.py        (Ctrl-C to stop; resumes karpathy_pong.pt)
"""
...
# ── hyperparameters — FASTER / more aggressive variant (vs realpong's baseline) ──
batch_size    = 5         # smaller batch -> more frequent updates (faster early progress)
learning_rate = 3e-3      # higher lr -> learns quicker (grad-clipped for stability)
gamma         = 0.99
value_coef    = 0.5
entropy_coef  = 0.01      # entropy bonus -> keeps exploring, avoids premature collapse
grad_clip     = 1.0       # clip gradients so the higher lr stays stable
SAVE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "karpathy_pong.pt")
SEED = 2
...
def main():
    ...
    ep = 0
    opt.zero_grad()
    print("training karpathy (FAST variant: lr 3e-3, batch 5, entropy) on pong_v3 (RIGHT) "
          "-- opponent: RANDOM (bootstrap) -- Ctrl-C to stop\n")
    try:
        while True:
            obs, _ = env.reset(seed=SEED + ep)
            prev = None
            logps, values, rewards, probs = [], [], [], []
            bf_last = None
            ...
            (policy_loss + value_loss - entropy_coef * entropy).backward()   # +entropy bonus

            ep += 1
            if ep % batch_size == 0:
                torch.nn.utils.clip_grad_norm_(net.parameters(), grad_clip)   # stabilize higher lr
